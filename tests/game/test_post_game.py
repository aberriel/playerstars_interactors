...
from playerstars_interactors.game.post_game import (
...
console = Console(entity_id='12345',
                  name="Xbox",
...
def test_post_game(resource):
    request = PostGameRequestModel(json_data)
    interactor = PostGameInteractor(request, adapter_mock, console)
...
@patch('boto3.resource')
def test_post_game_raises(resource):
    request = PostGameRequestModel(json_data)
    interactor = PostGameInteractor(request, adapter_mock_raises, console)
    with pytest.raises(SaveGameException) as excinfo:
        interactor.run()
