from playerstars_interactors import (
    GetMyTeamsRankingRequestModel, GetMyTeamsRankingInteractor
)
from unittest.mock import MagicMock
from tests.util_tests import team_list, team_5
...
team_adapter = MagicMock(filter=MagicMock(return_value=team_list + [team_5]))


def test_get_team_ranking():
    request = GetMyTeamsRankingRequestModel(
        params=params, playerd_id='q1w2e3abc')
    interactor = GetMyTeamsRankingInteractor(request, team_adapter)
    data, range_data = interactor.run()
    assert range_data.initial == 0
    assert range_data.final == 2
    assert range_data.total == 2
