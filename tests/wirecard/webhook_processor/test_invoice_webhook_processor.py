...
def processor_fixture(request):
    def factory(adapters: WebhookProcessorAdapters = MagicMock(),
        ...
        processor = InvoiceWebhookProcessor(
            adapters=adapters,
...
class TestInvoiceWebhookProcessor(TestCase):
    def setUp(self):
        fac = TestInvoiceWebhookProcessor.factory()
        self.processor: InvoiceWebhookProcessor = fac.processor
    ...
    @patch.object(InvoiceWebhookProcessor, '_get_subscription_from_api')
    @patch.object(InvoiceWebhookProcessor, '_get_player')
    @patch.object(InvoiceWebhookProcessor, '_get_plan_from_api')
    @patch.object(InvoiceWebhookProcessor, '_add_payment_log')
    @patch.object(InvoiceWebhookProcessor, '_log_received_webhook')
