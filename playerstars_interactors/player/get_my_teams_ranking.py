...
class GetMyTeamsRankingRequestModel:
...
class GetMyTeamsRankingResponseModel:
...
class GetMyTeamsRankingInteractor:
    def __init__(self, request: GetMyTeamsRankingRequestModel,
                 team_adapter):
    ...
    def format_response(self, teams: List[Team]):
        position = 1
        formated_teams = list()
        previous_team = None
        for team in teams:
    ...
    def filter_my_teams(self, teams):
        filtered_list = list()
        for team in teams:
            member_id_list = [x.player_id for x in team.get_active_members()]
            if self.request.player_id in member_id_list:
                filtered_list.append(team)
        return filtered_list

    def run(self):
        self.logger.info("COMEÇANDO GET RANKING MY TEAMS")
        ...
        self.logger.info("lista de times ordenada")

        filtered_my_teams = self.filter_my_teams(ordered_teams)
        self.logger.info("lista de times filtrada")

        formated_response = self.format_response(filtered_my_teams)
        ...
        range_data = get_partial_range(
            filtered_my_teams, self.request.pagination_page,
            self.request.pagination_per_page, 'ranking')

        response = GetMyTeamsRankingResponseModel(
